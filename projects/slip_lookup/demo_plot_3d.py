...
class PlotWindow(QMainWindow):
    ...
    def update_plot(self):
        ...
        VX, AX = np.meshgrid(self.vx_dense, self.ax_dense)
        points = np.stack([VX.ravel(), AX.ravel(), np.full(VX.size, sx_fixed)], axis=-1)
        Z = self.ax_lookup_dec(points).reshape(VX.shape)

        self.ax.clear()
        surf = self.ax.plot_surface(VX, AX, Z, cmap='jet')
        self.ax.set_xlabel('vx')
        self.ax.set_ylabel('ax')
        self.ax.set_zlabel(f'lookup value (sx={sx_fixed:.2f})')
        self.ax.set_title('ax_lookup_dec (interactive)')

        # 清除旧 colorbar
        self.cax.clear()
        self.cbar = self.fig.colorbar(surf, cax=self.cax)
        self.cbar.set_label('lookup value')
